    ...
    return (
        <div className="h-full flex flex-col">
            <div className="mb-6 flex justify-between items-center">
                <div>
                    <h1 className="text-2xl font-bold text-slate-900">Stories & Epics</h1>
                    <p className="text-slate-600">High-level project roadmaps</p>
                </div>
            ...
            <div className="flex gap-6 h-full overflow-x-auto pb-4">
                {columns.map(col => (
                    <div key={col.id} className={`flex-1 min-w-[350px] rounded-xl border border-slate-200 flex flex-col ${col.color}`}>
                        <div className="p-4 border-b border-slate-200 flex justify-between items-center">
                            <h3 className="font-bold text-slate-700">{col.title}</h3>
                            <span className="bg-slate-50 text-slate-600 text-xs px-2 py-1 rounded-full">
                        ...
                        <div className="p-4 space-y-4 overflow-y-auto flex-1">
                            {stories.filter(s => s.status === col.id).map(story => (
                                <div key={story.id} className="bg-slate-50 p-5 rounded-xl border border-slate-200 hover:border-purple-500/50 transition-all cursor-pointer group shadow-lg">
                                    ...
                                    <h4 className="text-lg font-semibold text-slate-900 mb-2">{story.title}</h4>
