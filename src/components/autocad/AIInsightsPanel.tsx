...
const AIInsightsPanel: React.FC<AIInsightsPanelProps> = ({ autocadFile }) => {
    ...
    return (
        <div className="bg-slate-50 border border-slate-200 rounded-xl overflow-hidden">
            {/* Header */}
            <div className="p-4 border-b border-slate-200 bg-gradient-to-r from-blue-500/10 to-purple-500/10">
                ...
                <div className="grid grid-cols-3 gap-3">
                    <div className="bg-white/50 p-3 rounded-lg">
                        <div className="text-xs text-slate-600 mb-1">Critical/High</div>
                        <div className="text-xl font-bold text-slate-900">{criticalCount + highCount}</div>
                    </div>
                    <div className="bg-white/50 p-3 rounded-lg">
                    ...
                    <div className="bg-white/50 p-3 rounded-lg">
                        ...
                        <div className="text-xl font-bold text-slate-900">{Object.keys(insightsByType).length}</div>
                    </div>
                </div>
            </div>
            ...
            <div className="p-4 space-y-4 max-h-[600px] overflow-y-auto">
                {insights.length === 0 ? (
                    <div className="text-center py-12">
                        ...
                        <p className="text-slate-600 text-sm">No AI insights available</p>
                    ...
                    insights.map((insight) => {
                        ...
                        return (
                            <div
                                key={insight.id}
                                className={`p-4 rounded-lg border ${severityColors[insight.severity]} bg-white/50`}
                            >
                                <div className="flex items-start justify-between mb-2">
                                    <div className="flex items-center gap-2">
                                        <div className={`p-2 bg-slate-50 rounded-lg ${typeColor}`}>
                                            <TypeIcon className="w-4 h-4" />
                                        </div>
                                        <div>
                                            <h4 className="text-sm font-semibold text-slate-900">{insight.title}</h4>
                                            <div className="flex items-center gap-2 mt-1">
                                                <span className="text-xs text-slate-600 capitalize">{insight.type}</span>
            ...
            {/* Footer Actions */}
            {insights.length > 0 && (
                <div className="p-4 border-t border-slate-200 bg-white/50 flex gap-2">
                    <button className="flex-1 bg-blue-600 hover:bg-blue-500 text-white px-4 py-2 rounded-lg text-sm transition-colors">
                        Export Report
                    </button>
                    <button className="flex-1 bg-white hover:bg-slate-100 text-slate-600 px-4 py-2 rounded-lg text-sm transition-colors">
                        Share Insights
                    </button>
                </div>
            )}
